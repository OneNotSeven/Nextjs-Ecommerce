...
const Women = async() => {
    ...
    try {
      dataProducts = await apiProductsCategory("womens-bags");
      datarr = [dataProducts]
            ...
            
              {
                ...
                dataSliced.map((products, i) => (
                  <Link key={i} href={`/users/${products.title}/` + `${products.id}`}>
                    <div key={i} className=" relative group aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg  xl:aspect-h-8 xl:aspect-w-7  bg-[#212121] shadow-md">
                      <Image key={i}
                      ...
                      <button key={i} className="btn hidden  bg-gray-950 group-hover:block hover:bg-gray-800 rounded-tr-2xl absolute w-40 bottom-0 p-3 text-white">Buy Now</button>
                    </div>
